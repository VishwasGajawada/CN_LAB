 ...
 */
int send_arp(int fd, int ifindex, const unsigned char *src_mac, uint32_t dst_ip)
{
    int err = -1;
    ...
    uint32_t src_ip = htonl(0xac1e8b64);

    debug("Copy IP address to arp_req");
    memcpy(arp_req->sender_ip, &src_ip, sizeof(uint32_t));
    dst_ip = htonl(0xac1e8806);
...
int test_arping(const char *ifname, const char *ip)
{
    ...
    if (send_arp(arp_fd, ifindex, mac, dst))
    {
