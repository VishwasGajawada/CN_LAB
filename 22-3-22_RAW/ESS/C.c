...
void* send_worker(void *arg) {
    ...
    // for(int i=0; i<4; i++) {
        // if(i_am_running) {
            ...
            sprintf(send_msg, "I am client %d, I am running currently\n", client_number);
...
void* recv_worker(void *arg) {
    ...
    while(previous_client_ready_to_accept == -1) {
        char buf[1024];
        recvfrom(rsfd, buf, sizeof(buf), 0, NULL, NULL);
        // perror("recvfrom");
...
int main(int argc, char **argv) {
    ...
    pthread_t pid1;
    pthread_create(&pid1, NULL, recv_worker, NULL);
    ...
    if( msgsnd(msqid, &msg, sizeof(msg), 0) < 0)
        handle_error("msgsnd");
    printf("Pid added to message queue\n");
    ...
    pthread_t pid2;
    pthread_create(&pid2, NULL, send_worker, &client_number);
